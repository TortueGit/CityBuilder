...
void Gui::highlight(const int entry)
{
	for (int i = 0; i < entries.size(); ++i)
	{
		if (i == entry)
		{
			...
			entries[i].text.setFillColor(style.textHighlightBodyCol);
			entries[i].text.setOutlineColor(style.textHighlightBorderCol);
		}
		else
		{
			entries[i].shape.setFillColor(style.bodyCol);
			entries[i].shape.setOutlineColor(style.borderCol);
			entries[i].text.setFillColor(style.textBodyCol);
			entries[i].text.setOutlineColor(style.textBorderCol);
		}
	}
