...
double City::distributePool(double& pool, Tile& tile, double rate = 0.0)
{
	...
	/* If there is room in the zone, move up to 4 people frm the
	* pool into the zone */
	if (pool > 0)
	{
		...
		if (std::getline(lineStream, key, '='))
		{
			...
			if (std::getline(lineStream, value))
			{
				if (key == "width")
					...
					this->populationPool = std::stoi(value);
				else if (key == "employmentPool")
					this->employmentPool = std::stoi(value);
				else if (key == "population")
					this->population = std::stoi(value);
				else if (key == "employable")
					this->employable = std::stoi(value);
				else if (key == "birthRate")
					this->birthRate = std::stoi(value);
				else if (key == "deathRate")
					this->deathRate = std::stoi(value);
				else if (key == "residentialTax")
					this->residentialTax = std::stoi(value);
				else if (key == "commercialTax")
					this->commercialTax = std::stoi(value);
				else if (key == "industrialTax")
					this->industrialTax = std::stoi(value);
				else if (key == "funds")
					this->funds = std::stoi(value);
				else if (key == "earnings")
					this->earnings = std::stoi(value);
			}
			else
			{
...
void City::update(float dt)
{
	...
	for (int i = 0; i < this->map.tiles.size(); ++i)
	{
		...
		{
			/* Redistribute the pool and increase the population total by the tile's poputlation. */
